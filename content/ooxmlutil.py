...
# (c) Craig Duncan 2019-2020.  This update 10 Sep 2020.
# 
...
# public function to return the document.xml content of a given .docx file
# input args are (1) filepath of .docx file (2) path of XML file inside docx zip file
# Examples of second argument:
# worddoc='word/document.xml'
# numberdoc='word/numbering.xml'
# styledoc = 'word/styles.xml'
...
# 1 old docx filepath
# 2 new docx filepath
# 3 string content of the replacement document.xml to be replaced
...
# 1 old docx filepath
# 2 new docx filepath
# 3 string content of the replacement styles.xml file to be replaced
def writeNewStyleFileZip(nbpath,newname,stylesxml):
    maindoc='word/styles.xml'
    excludednames = [maindoc] 
    nbpackages=_getZipInfo(nbpath) 
    unzipsrc=zipfile.ZipFile(nbpath,'r',zipfile.ZIP_DEFLATED)
    with zipfile.ZipFile(newname, 'w',zipfile.ZIP_DEFLATED) as zipwrite:
        ...
        for item in nbpackages:
            if item.filename not in excludednames:  # no duplication
                ...
                print("notebook package not re-saved:"+item.filename)
                zipwrite.writestr(item,stylesxml)       
        zipwrite.close()
